...
### part 6
  ...
  - 哈希函数：
       ...
       - 输出域上的值均匀分布
  - 哈希表
       - 数组加链表实现，初始化时给一个定长为 N 的数组 H，put(key, value)时，计算index=hash(key)%N，将该记录挂在数组中H[index]的链表上（java中是一棵红黑树）。根据hash函数的性质，每一个数据元素上挂载的链表的长度应该是几乎相等的，当该长度大于阈值时候，进行hash表的扩容。
  实例：一个大小为100T的文件，每行一个字符串文本，如何统计重复的字符串文本。\
